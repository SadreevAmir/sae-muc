...
def get_available_servers():
    # Colab / non-SLURM fallback:
    # If users run vLLM locally (e.g. `--port 8000`) and set `VLLM_URL`,
    # we can skip `squeue` discovery entirely.
    ...
    try:
        ...
        raise RuntimeError(
            "Can't run `squeue` (non-SLURM environment). "
            "Set `VLLM_URL` (e.g. http://localhost:8000/v1) or run inside SLURM."
        ) from e
    ...
    if not server_dict:
        raise RuntimeError(
            "No running vLLM servers discovered via `squeue`. "
            "Set `VLLM_URL` (e.g. http://localhost:8000/v1) for Colab runs."
        )
